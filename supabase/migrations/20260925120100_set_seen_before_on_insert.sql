-- Record at insert time whether the website was already known.
--
-- 20260925120000 added `seen_before` and backfilled it from the profile's
-- first sighting. New rows should not depend on that heuristic: at the moment
-- of the insert we know exactly which profiles this job created, because they
-- are in v_new_profiles. Anything outside that array predates the scrape.
  ...
  if p_results is not null and jsonb_typeof(p_results) = 'array' then
    ...
    with ins as (
      ...
      select
        ...
        -- Did this website already have a profile before this job ran? The
        -- profiles created a few statements ago are in v_new_profiles, so
        -- anything NOT in that array predates this scrape. Exact, and it
        -- avoids the timestamp-slack heuristic the backfill had to use.
        (l.profile_id is not null and not (l.profile_id = any(v_new_profiles)))
