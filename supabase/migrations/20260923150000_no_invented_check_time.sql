-- Never invent a "last checked" time.
--
-- The inherited monday_checked_at fell back to now() when the profile had
-- a verdict but no match time. That case does not occur today — every
-- profile with a Monday verdict carries a real monday_matched_at — but the
-- fallback would have stamped the badge's tooltip with the moment of the
-- insert and called it a check. A missing time should read "never
-- checked", which is true, rather than "checked just now", which is not.
