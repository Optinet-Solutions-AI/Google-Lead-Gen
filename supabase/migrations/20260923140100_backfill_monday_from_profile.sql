-- Bring the rows already written into line with their profiles.
--
-- Two parts. First replay_missing_leads picks up the Monday inheritance
-- added to complete_scrape_job, so a future replay does not reintroduce
-- the gap. Then the rows written before that fix take the profile's
-- answer, which they should have had all along.
--
-- Deliberately narrow: only rows that were never checked (monday_checked_at
-- is null AND is_on_monday is null) and whose profile has a verdict. A row
-- an operator overrode by hand, or one the matcher has genuinely examined,
...
begin
  ...
  set is_on_monday         = p.is_on_monday,
      ...
      monday_checked_at    = coalesce(p.monday_matched_at, now()),
      monday_overridden_at = p.monday_overridden_at
  from public.website_profiles p
  where p.id = g.profile_id
    and g.monday_checked_at is null
    and g.is_on_monday is null
    and g.monday_overridden_at is null
    and p.is_on_monday is not null;
  get diagnostics v_n = row_count;
  raise notice 'monday inherited onto % lead rows', v_n;
end $$;
