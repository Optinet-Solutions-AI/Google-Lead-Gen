-- Inherit the Monday match, like every other verdict.
--
-- complete_scrape_job already copies the profile's affiliate, Rooster,
-- contact, s-tag, system-flag and not-relevant verdicts onto each new lead
-- row. The Monday match was the one it left out, so every freshly scraped
-- row began with is_on_monday = null and waited for a separate matcher
-- pass to fill it in.
--
-- With the "Already exists?" column that gap became visible and wrong: a
-- website we have known to sit on a Monday board for months came back
-- from a scrape reading "In system", because the lead row itself had
-- never been checked. 294 of 317 unchecked rows were on Monday according
-- to their own profile.
--
-- The profile is the canonical record and is what the Monday sync keeps
-- current, so read the answer from it at insert time. monday_checked_at
-- carries the profile's match time rather than now(), so the badge's
-- tooltip reports when we actually last looked. A null profile verdict
-- still inherits as null — unknown stays unknown, and the matcher fills
-- it in as before.
--
-- Only the two Monday clauses change; the rest is the deployed definition.
  ...
  if p_results is not null and jsonb_typeof(p_results) = 'array' then
    ...
    with ins as (
      ...
      select
        ...
        p.monday_item_id,
        p.monday_match_kind,
        case when p.is_on_monday is not null then coalesce(p.monday_matched_at, now()) end,
        p.monday_overridden_at,
