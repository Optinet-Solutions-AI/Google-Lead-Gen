...
export type JobsQueryResult = {
  rows: ScrapeJob[]
  total: number
  /** How the advanced search read the query ("norway" -> country NO), so the
   *  page can show its interpretation rather than leaving it a black box. */
  searchNotes?: string[]
}
...
export async function queryJobs(opts: JobsQueryOptions): Promise<JobsQueryResult> {
  ...
  // on the row; words are expanded first (country names, status/engine
  // synonyms, batch numbers, and the domains a batch produced).
  let searchNotes: string[] = []
  if (opts.q && opts.q.trim().length > 0) {
    const plan = await buildJobSearch(svc, opts.q)
    if (plan) {
      // Repeated `or=` parameters are ANDed by PostgREST, which is exactly
      // "every word has to match, but each may match a different column".
...
async function fetchEnrichmentStatus(
  ...
  const data = (await selectInChunks(jobIds, chunk =>
    svc
      .from('google_lead_gen_table')
      .select(
        'id, scrape_job_id, is_on_monday, affiliate_checked_at, rooster_checked_at, contact_checked_at, s_tags_checked_at, is_not_relevant, system_flag',
      )
      .in('scrape_job_id', chunk),
  )) as unknown as LeadRow[]

  // Counted from the rows we already fetched for the pipeline dots, so
  // an accurate Results column costs no extra round trip.
  for (const jobId of jobIds) {
    counts.set(jobId, { stored: 0, visible: 0, notRelevant: 0, flagged: 0 })
  }
  for (const row of data) {
    const jobId = row.scrape_job_id
    if (!jobId) continue
